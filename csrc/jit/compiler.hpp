...
public:
    ...
    Compiler() {
        ...
        for (const auto& name : {"EP_PROFILE_QUIET", "EP_TAIL_ADAPTIVE", "EP_ASYNC_EXIT", "EP_FAST_ENTRY", "EP_PREWARM_FLUSH"}) {
            if (get_env(name, 0))
        ...
        for (const auto& name : {"EP_NUM_SUB_PARTS", "EP_MIN_SUB_TOKENS", "EP_NUM_SUB_PARTS_LAST",
                                 ...
                                 "EP_PULL_FORWARD", "EP_WARM_PUT", "EP_FWD_CHUNK", "EP_SLIM_WIRE", "EP_COMPRESS_META"}) {
